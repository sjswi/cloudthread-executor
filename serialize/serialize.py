...
from serialize.utils import bytes_to_b64str
...
class SerializeIndependent:
    ...
    def __call__(self, list_of_objs, include_modules, exclude_modules):
        ...
        for obj in list_of_objs:
            if type(obj) == tuple:
                for object in obj:
                    mods.extend(self._module_inspect(object))
        ...
        # 直接返回moduledata
        data = create_module_data(list(mod_paths))
        return (strs, data)
...
# 返回字典，文件名，文件内容
def create_module_data(mod_paths: list) -> dict:
    module_data = {}
    # load mod paths
    for m in mod_paths:
        if os.path.isdir(m):
